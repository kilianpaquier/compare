...
// ErrorAs asserts that err can be assigned to target.
func ErrorAs[T any](t testing.TB, err error, target T) {
	t.Helper()
	if !errors.As(err, target) {
		t.Errorf("expected error of type '%T', got '%T'", target, err)
	}
}
